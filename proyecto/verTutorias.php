...
<?php
	...
	$query = "SELECT OID_T, Fecha, Hora_Comienzo, Duracion, Estado, DNI_Alum FROM Tutorias WHERE (DNI_Prof = '$DNI')";
...
?>
	...
	<div class="tutorias">
    	...
    	<table>
    		<tr>
				<th>Fecha</th> <th>Hora de comienzo</th> <th>Duración</th> <th>Alumno</th> <th>Estado</th>
			</tr>
    	<?php foreach($tutorias as $tutoria){?>
    		<tr class="libro">
				...
				<td> <?php  $nombre = nombreAlumno($tutoria['DNI_ALUM'], $conexion);
					echo $nombre['NOMBRE'];
					cerrarConexionBD($conexion);
				?></td>
				<td> <?php echo $tutoria['ESTADO'] ?></td>
				<?php 	
					if($tutoria["ESTADO"]=='Pendiente' || isset($_SESSION["cambiar"])){
						if(isset($_SESSION["cambiar"])){
							unset($_SESSION["cambiar"]);
						}
					?>		 
						<td><button name="confirmar" type="submit" value="<?php echo $tutoria['OID_T']?>" >
								<img src="img/confirmar.png"  alt="confirmar" style=" width: 20%; height: 20%;">
							</button>
						</td>
						<td><button name="denegar" type="submit" value="<?php echo $tutoria['OID_T']?>" >
								<img src="img/denegar.png"  alt="denegar" style=" width: 20%; height: 20%;" >
							</button>
						</td>
			<?php	
					}else{
			?>
						<td><button name="cambiar" type="submit"  >
								Cambiar
							</button>
						</td>
